...
log.basicConfig(filename='app.log',filemode='a',format="%(name)s - %(levelname)s - %(message)s")
class KeyWrapper:
...
class Isis:
    ...
    def deliverMsg(self,Msg):
        ...
        #print("after the deliver")import heapq
import bisect 
class KeyWrapper:
    def __init__(self, iterable, key):
        self.it = iterable
...
class Isis:
    ...
    def deliverMsg(self,Msg):
        ...
        for pair in self.queue:
            log.warning(f'      {pair[1].id} {pair[1].deliverable} {pair[0]}')
        return deliverMsgs
